...
Build a maintainable multi-tenant Real Estate CRM SaaS. The application allows real estate companies to manage:
...
Use a modular monolith with Clean Architecture. Do not use microservices initially.
...
React + TypeScript SPA. Frontend communicates with ASP.NET Core through HTTPS REST APIs. SignalR is used where realtime communication provides value.
...
All of the items originally listed here (AI Lead Assistant, WhatsApp automation, subscriptions, online payments, mobile apps, public marketplace, advanced recommendation engine, external developer APIs) have since been explicitly requested and implemented — see `docs/roadmap.md` Phases 19–21 for what each one is and how it's built. Nothing remains deferred. Add new items here only if the user again defers something they don't want built yet.
